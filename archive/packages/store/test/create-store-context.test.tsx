...
describe('local component store', () => {
	...
	test('should return withProvider prop', async () => {
		const globalUserStore = store({
			name: '',
			age: 25,
		}).extend((store) => ({
...
describe('should integrate with actions, effects and computed', () => {
	...
	test('computed values should be scoped to each component', async () => {
		...
		await waitFor(() => {
			expect(ui.getByTestId(`${testIds.doubledCount}-comp1`)).toHaveTextContent(
				'Doubled Count: 12'
			);
		});
		await waitFor(() => {
			...
			expect(ui.getByTestId(`${testIds.doubledCount}-comp2`)).toHaveTextContent(
				'Doubled Count: 20'
			);
		});
	});
	...
	test('effects should be scoped to each component', async () => {
		...
		await waitFor(() => {
			expect(ui.getByTestId(`${testIds.synchedCount}-comp1`)).toHaveTextContent(
				'Synched Count: 6'
			);
			expect(ui.getByTestId(`${testIds.synchedCount}-comp2`)).toHaveTextContent(
				'Synched Count: 9'
			);
		});
	});
