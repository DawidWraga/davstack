...
- Davstack Store is built on top of Zustand and immer. It is recommended to familiarize yourself with these libraries to understand how Davstack Store works under the hood. In particular, see [https://immerjs.github.io/immer/pitfalls](https://immerjs.github.io/immer/pitfalls) for common pitfalls when using immer.
- If you are using an array, it is recommended to nest the array inside an object for example, avoid using `store([])` and instead use `store({ items: [] })`. Otherwise you may experience some reactivity issues.
- Only the first two levels of nesting will have autogenerated methods. This is intentional to optimize performance incase of deeply nested objects. However, a proxy object could be added to the store to access deeper nested properties. Contributions are welcome to improve this feature.
